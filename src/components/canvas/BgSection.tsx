...
export default function BgSection({
  ...
  const [tab, setTab] = useState<'color' | 'image'>(imageUrl ? 'image' : 'color')
          ...
          <button
            ...
            className={`px-3 py-1 transition-colors ${
              tab === 'color'
                ...
                : 'bg-white text-text-muted hover:text-text-main'
            }`}
          >
            Color
          </button>
      ...
      {tab === 'image' && (
        <div className="space-y-2">
          {imageUrl ? (
            ...
            <button
              onClick={() => inputRef.current?.click()}
              className="touch-target w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed border-border hover:border-primary hover:bg-primary/5 text-xs font-medium text-text-muted hover:text-primary transition-all"
            >
