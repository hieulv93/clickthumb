      ...
      <article>
        <header className="mb-10">
          ...
          <p className="text-lg text-text-muted leading-relaxed">
            Instagram supports multiple image formats — square, portrait,
            landscape, Stories, Reels — each with different dimensions and
            cropping behavior. Using the wrong size means Instagram re-crops or
            compresses your image, reducing quality. Here are the exact
            specifications for every format in 2026.
          </p>
        </header>
        ...
        <div className="space-y-10 text-text-main leading-relaxed">
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-surface">
                    <th className="text-left p-3 font-semibold text-text-main border border-border">
                      Format
                    </th>
                    <th className="text-left p-3 font-semibold text-text-main border border-border">
                      Dimensions
                ...
                <tbody className="text-text-muted">
                  {[
                    [
                      ...
                      "1080 × 1080 px",
                      "1:1",
                      "Feed posts, brand content",
                    ],
                    [
                      "Portrait post",
                      "1080 × 1350 px",
                      "4:5",
                      "Photos, takes more feed space",
                    ],
                    [
                      "Landscape post",
                      "1080 × 566 px",
                      "1.91:1",
                      "Wide photos, panoramas",
                    ],
                    [
                      "Story / Reel",
                      "1080 × 1920 px",
                      "9:16",
                      "Vertical video & images",
                    ],
                    [
                      "Carousel slide",
                      "1080 × 1080 px",
          ...
          <section className="space-y-4">
            ...
            <p className="text-text-muted">
              Instagram supports three aspect ratios for feed posts. Choosing
              the right one affects how much of the feed your post occupies —
          ...
          <section className="space-y-4">
            ...
            <p className="text-text-muted">
              Your Instagram profile grid displays 9–12 posts visible without
              scrolling — this is the first impression for new visitors.
              Creators who grow fastest on Instagram treat the grid as a
              designed layout, not a random collection. A cohesive grid converts
          ...
          <section className="space-y-4">
            ...
            <p className="text-text-muted">
              Instagram re-compresses every uploaded image. Even if you upload a
              perfectly sharp 1080×1080 JPEG, Instagram will run it through its
              own compression algorithm — often reducing quality noticeably,
              especially in areas with fine detail or gradients.
            </p>
            <p className="text-text-muted">
            ...
            <div className="space-y-3">
              {[
                {
                  tip: "Upload at exactly the right pixel dimensions",
                  desc: "If Instagram has to resize your image (upscale or downscale), it re-encodes the JPEG, compressing twice. Upload at exactly 1080px width to avoid the resize step.",
                },
                {
                  tip: "Pre-compress before uploading",
                  desc: "Instagram's compression is aggressive on large files. Pre-compressing your image to ~500–800KB before upload means Instagram's algorithm has less work to do — the result tends to be sharper. Use a tool like compressimg.pro to reduce file size before uploading.",
                },
              ].map(({ tip, desc }) => (
          ...
          <section className="space-y-4">
            ...
            <p className="text-text-muted">
              Stories and Reels use the same dimensions:{" "}
              <strong className="text-text-main">
                1080×1920 pixels (9:16)
              </strong>
              . This is the full-screen portrait format that fills the entire
              phone display.
            </p>
            <div className="bg-surface border border-border rounded-xl p-5 text-sm space-y-2">
              ...
              {[
                [
                  "Top 250px",
                  "Instagram UI overlay (profile name, music label) — avoid placing content here",
                ],
                [
                  "Bottom 250px",
                  "Like/comment buttons and caption — avoid placing content here",
                ],
                [
                  "Center 1080×1420px",
                  "Safe zone — all text, faces, and key elements here",
                ],
              ].map(([zone, note]) => (
                <div key={zone} className="flex gap-3 text-text-muted">
                  <span className="font-medium text-text-main w-36 shrink-0">
                    {zone}
                  </span>
                  <span>{note}</span>
                </div>
          ...
          <section className="space-y-4">
            ...
            <p className="text-text-muted">
              ...
              opens at 1080×1080 — the standard square format. No signup, no
              watermark, exports at full resolution instantly.
            </p>
            <div className="space-y-3">
              {[
                {
                  step: "Step 1",
                  title: "Open Instagram Post Maker",
                  desc: "Canvas loads at 1080×1080 (square format). Pick a template that suits your post type — product showcase, quote graphic, event announcement, or brand post.",
                },
                {
                  step: "Step 2",
                  title: "Add your content",
                  desc: "Replace template text with your message. Upload your image as the background. Adjust the background color to match your brand palette.",
                },
                {
                  step: "Step 3",
                  title: "Check text position",
                  desc: "Ensure all text sits within the center of the canvas — not touching edges. Instagram may crop a small amount on some devices.",
                },
                {
                  step: "Step 4",
                  title: "Download and upload",
                  desc: "Export as JPG (1080×1080). Upload directly to Instagram. The file size will typically be 200–500KB, well under Instagram's 30MB limit.",
                },
              ].map(({ step, title, desc }) => (
                <div
                  key={step}
              ...
              {[
                {
                  ...
                  a: "1080×1080px (square, 1:1) is the safest universal choice. For maximum feed presence, use 1080×1350px (portrait, 4:5) — it occupies 35% more vertical space and typically drives more engagement.",
                },
                {
                  q: "Why does Instagram crop my photos?",
                  a: "Instagram crops when your image aspect ratio is outside the supported range (0.8:1 to 1.91:1). Portrait images taller than 4:5 get cropped to 4:5. Use 1080×1350 (4:5) for the tallest supported portrait format without cropping.",
                },
                {
                  q: "Does image size affect Instagram reach?",
                  a: "Image dimensions don't directly affect reach. But image quality does — blurry or heavily compressed images get less engagement, which signals lower quality to the algorithm. Upload sharp, correctly-sized images.",
                },
                {
                  q: "What is the Instagram carousel size?",
                  a: "1080×1080px (square) per slide is the standard. All slides must use the same aspect ratio — Instagram will force them to match the first slide's ratio. Design all carousel slides at 1080×1080 for consistency.",
                },
                {
                  q: "Should I post portrait or square on Instagram?",
                  a: "Portrait (1080×1350) generally outperforms square because it occupies more screen space in the feed. Use portrait for photos of people, food, and text graphics. Use square when you need a consistent grid layout or when the image composition doesn't work in portrait.",
                },
                {
                  q: "What resolution should Instagram posts be?",
                  a: "Upload at 1080px wide for all Instagram formats: 1080×1080 (square), 1080×1350 (portrait), or 1080×1920 (Stories/Reels). Instagram does not improve quality above 1080px — anything wider gets downscaled. Uploading at exactly 1080px avoids the double-compression penalty of resize + encode.",
                },
                {
                  q: "How do I stop Instagram from compressing my photos?",
                ...
                {
                  q: "Does posting time affect how my Instagram post looks?",
                  a: "Posting time affects reach, not image quality. Instagram's compression is applied equally regardless of when you post. For reach, posting when your audience is most active (check Insights for your account's peak hours) typically results in more initial engagement, which signals to the algorithm to distribute the post further.",
                },
              ].map(({ q, a }) => (
                <div
                  key={q}
